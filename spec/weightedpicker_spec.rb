...
describe "Weightedpicker::initialize" do
  before do
    @wp01 = WeightedPicker.new("spec/a256b128.yaml", ["A", "B"])
    @wp03 = WeightedPicker.new("spec/a512b64.yaml", ["A", "B"])
  ...
  it "should merge when keys between weights and items" do
    ...
    WeightedPicker.new("spec/a512b64.yaml", ["B","C"]).weights.should == { "B" => 64, "C" => 256, }
...
#    #YAML.load_file(AB_YAML).should == { "A" => 256, "B" => 128}
  ...
  it "should raise exception if include not integer weight." do
    weights = { "A" => 1.0, "B" => 0.5, }
    lambda{
      WeightedPicker.new("spec/float.yaml", ["A","B"])
...
describe "Weightedpicker::pick" do
  before do
    @wp01 = WeightedPicker.new(AB_YAML, ["A", "B"])
  end

  it "should pick" do
    wp10 = WeightedPicker.new(AB_YAML, [])
    lambda{wp10.pick}.should raise_error(WeightedPicker::NoEntryError)
...
describe "include zero weight" do
  it "should not change zero weight." do
    wp01 = WeightedPicker.new("spec/a256b0.yaml", ["A", "B"])
...
describe "include one weight" do
  it "should not change zero weight." do
    wp01 = WeightedPicker.new("spec/a256b1.yaml", ["A", "B"])
